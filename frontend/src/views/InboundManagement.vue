<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div><h2 class="text-2xl font-bold text-slate-800">入库管理</h2><p class="text-sm text-slate-500 mt-0.5">管理仓库入库记录</p></div>
    ...
    <FormModal :visible="modalVisible" :title="editingItem ? '编辑入库' : '新建入库'"
    ...
    <ConfirmDialog :visible="delVisible" title="删除入库记录" message="确定要删除该入库记录吗？" confirmText="删除" :danger="true"
...
const columns = [
  ...
  { key: 'inbound_no', label: '单号' },
  { key: 'supplier_name', label: '供应商' },
  { key: 'warehouse_name', label: '仓库' },
  { key: 'total_quantity', label: '数量' },
  { key: 'inbound_date', label: '日期', render: (v) => formatDate(v) },
...
async function handleSubmit(data) {
  ...
  try {
    if (editingItem.value) { await updateInboundRecord(editingItem.value.inbound_record_id, data); }
    else { await createInboundRecord(data); }
...
async function handleDelete() {
  ...
  try { await deleteInboundRecord(delItem.value.inbound_record_id); delVisible.value = false; delItem.value = null; tableRef.value?.fetchData(); }
  catch (e) { toast.error(e.message); }
}
</script>
